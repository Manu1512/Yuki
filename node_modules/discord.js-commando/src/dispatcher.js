			...
			if(!inhibited) {
				if(cmdMsg.command) {
					if(!cmdMsg.command.isEnabledIn(message.guild)) {
						responses = await cmdMsg.reply(`Der Befehl \`${cmdMsg.command.name}\` ist deaktiviert.`);
					...
					if(this.client.options.unknownCommandResponse) {
						responses = await cmdMsg.reply(
							`Unbekannter Befehl. Verwende ${cmdMsg.anyUsage(
								'help',
								message.guild ? undefined : null,
								message.guild ? undefined : null
							)}, um eine Liste mit allen Befehlen anzuzeigen.`
						);
					}
				}
			} else {
