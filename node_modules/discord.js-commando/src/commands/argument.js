	...
	async obtain(msg, value, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.reply(stripIndents`
				${empty ? this.prompt : valid ? valid : `Du hast ein ungültiges ${this.label} angegeben. Bitte versuche es nochmal.`}
				${oneLine`
					Antworte mit \`cancel\`, um den Befehl abzubrechen.
					${wait ? `Der Befehl wird automatisch in ${this.wait} Sekunden abgebrochen.` : ''}
				`}
			`));
		...
		while(true) { // eslint-disable-line no-constant-condition
			...
			while(!valid || typeof valid === 'string') {
				...
				if(value) {
					...
					prompts.push(await msg.reply(stripIndents`
						${valid ? valid : oneLine`
							Du hast ein ungültiges ${this.label} angegeben,
							"${escaped.length < 1850 ? escaped : '[Zu lang, um es anzuzeigen]'}".
							Bitte versuche es erneut.
						`}
						${oneLine`
							Antworte mit \`cancel\`, um den Befehl abzubrechen oder \`finish\`, um die Eingabe bis zu diesem Punkt abzuschließen.
							${wait ? `Der Befehl wird automatisch in ${this.wait} Sekunden abgebrochen.` : ''}
						`}
					`));
					...
					prompts.push(await msg.reply(stripIndents`
						${this.prompt}
						${oneLine`
							Antworte mit \`cancel\` um den Befehl abzubrechen oder \`finish\`, um die Eingabe zu beenden.
							${wait ? `Der Befehl wird automatisch in ${this.wait} Sekunden abgebrochen, außer du schreibst zurück.` : ''}
						`}
					`));
				}
