...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `Der Befehl \`${this.name}\` kann nur vom Bot-Besitzer verwendet werden.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `Der Befehl \`${this.name}\` erfordet die Berechtigung "${permissions[missing[0]]}".`;
				}
				return oneLine`
					Für den Befehl \`${this.name}\` werden folgende Berechtigungen benötigt:
