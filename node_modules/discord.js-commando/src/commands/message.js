...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`Der Befehl \`${this.command.name}\` muss auf einem Server verwendet werden.`);
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.reply(`Der Befehl \`${this.command.name}\` kann nur in NSFW Channels verwendet werden.`);
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			else return this.reply(`Du hast keine Berechtigungen, um den Befehl \`${this.command.name}\` zu verwenden.`);
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				if(missing.length === 1) {
					return this.reply(
						`Ich benötige die Berechtigung "${permissions[missing[0]]}", damit der Befehl \`${this.command.name}\` funktioniert.`
					);
				}
				return this.reply(oneLine`
					Ich brauche folgende Berechtigungen, damit der Befehl \`${this.command.name}\` funktioniert:
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return this.reply(
				`Du darfst den Befehl \`${this.command.name}\` für weitere ${remaining.toFixed(1)} Sekunden nicht verwenden.`
			);
		}
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				...
				return this.reply('Befehl wurde abgebrochen');
			}
			args = result.values;
		}
		...
		try {
			...
			if(err instanceof FriendlyError) {
				...
				return this.reply(stripIndents`
					...
					Du solltest niemals so einen Fehler erhalten.
					Bitte kontaktiere ${ownerList || 'den Bot-Besitzer'}${invite ? ` auf diesem Server: ${invite}` : '.'}
				`);
			}
		}
	}
